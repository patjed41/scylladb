...
async def test_raft_recovery_user_data(manager: ManagerClient, remove_dead_nodes_with: str, send_to_coordinator: bool):
    ...
    rf: int = 3
    ks_name = unique_name()
    finish_writes = await start_writes(cql, rf, ConsistencyLevel.LOCAL_QUORUM, 5, ks_name, True)
    ...
    finish_writes_dc1 = await start_writes(dc1_cql, rf, ConsistencyLevel.LOCAL_QUORUM, 3, ks_name)
    ...
    finish_writes_dc2 = await start_writes(dc2_cql, rf, ConsistencyLevel.LOCAL_QUORUM, 3, ks_name)
