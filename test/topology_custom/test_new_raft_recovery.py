...
from test.topology.util import delete_raft_data_and_upgrade_state, reconnect_driver
...
@pytest.mark.asyncio
async def test_new_raft_recovery(manager: ManagerClient):
    """
    ...
    for h in hosts:
        # I need to investigate what part of delete_raft_data_and_upgrade_state is necessary.
    ...
    for i, srv in enumerate(dead_servers):
        ignore_dead = [dead_srv.ip_addr for dead_srv in dead_servers[i + 1:]]
        await manager.remove_node(initiator_id=servers[0].server_id, server_id=srv.server_id, ignore_dead=ignore_dead)
