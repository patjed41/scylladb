...
async def test_zero_token_nodes_topology_ops(manager: ManagerClient, tablets_enabled: bool):
    # Test:
    # - topology operations in the Raft-based topology involving zero-token nodes
    # - that zero-token nodes preserve the Raft majority
    # - client requests to normal nodes in the presence of zero-token nodes (2 normal-nodes, RF=2, CL=2)
    ...
    # FIXME: Finish writes after the last server_add call when https://github.com/scylladb/scylladb/issues/19737 is fixed.
    ...
    await manager.server_add(replace_cfg_e, config=zero_token_cfg, expected_error='Cannot replace the token-owning node')
