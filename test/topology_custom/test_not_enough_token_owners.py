...
async def test_not_enough_token_owners(manager: ManagerClient):
    ...
    normal_cfg = {'enable_tablets': True}
    zero_token_cfg = {'enable_tablets': True, 'join_ring': False}
    ...
    # The second server is needed to preserve the Raft majority.
    server_b = (await manager.servers_add(2, config=zero_token_cfg))[0]
    ...
    ks_name = unique_name()
    await cql.run_async(f"CREATE KEYSPACE {ks_name} WITH replication = {{'class': 'NetworkTopologyStrategy', 'replication_factor': 2}} AND tablets = {{ 'enabled': true }}")
    ...
    # TODO: When https://github.com/scylladb/scylladb/issues/16195 is fixed, we will have to replace the expected error
    # message. Similar change may be needed for remove_node below.
