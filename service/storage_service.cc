    ...
    auto process_normal_node = [&] (raft::server_id id, const replica_state& rs) -> future<> {
        ...
        if (is_me(host_id)) {
            ...
            co_await _gossiper.add_local_application_state({
                { gms::application_state::TOKENS, gms::versioned_value::tokens(rs.ring.tokens) },
                ...
                { gms::application_state::STATUS, gms::versioned_value::normal(rs.ring.tokens) }
            });
        } else if (ip && !is_me(*ip)) {
        ...
        switch (cmd.cmd) {
            ...
            case raft_topology_cmd::command::stream_ranges: {
                co_await with_scheduling_group(_db.local().get_streaming_scheduling_group(), coroutine::lambda([&] () -> future<> {
                    ...
                    if (rs.ring.tokens.empty()) {
                        rtlogger.info("the node got stream_ranges request but it does not own any tokens");
                        co_return;
                    }
                    if (tstate != topology::transition_state::write_both_read_old && rs.state != node_state::normal && rs.state != node_state::rebuilding) {
                        rtlogger.warn("got stream_ranges request while my tokens state is {} and node state is {}", tstate, rs.state); // tokens -> topology
                        co_return;
                    }
