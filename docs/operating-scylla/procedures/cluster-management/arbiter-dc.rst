=========================================================
Preventing Quorum Loss in Two-DC Clusters
=========================================================

ScyllaDB requires at least a quorum (majority) of Raft voters in a cluster to
be up and communicate with each other. A cluster that loses a quorum of voters can
handle reads and writes of user data, but cluster management operations, such
as schema and topology updates, are impossible. For more information about the
quorum requirement and Raft voters see :ref:`Quorum Requirement <raft-quorum-requirement>`.

In clusters that have two (DCs), losing a quorum may occur if one of the DCs
becomes unavailable. For example, if one of the two DCs contains three out of five voters,
quorum is lost when that DC is down.
